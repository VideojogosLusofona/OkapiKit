    ...
    public class OkapiConfigEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (WriteTitle())
            {
                ...
                EditorGUILayout.PropertyField(props["_orderMode"], new GUIContent("Order mode", "Should the order in layer or the Z position changed when ordering by Y?"));

                if (props["_orderMode"].enumValueIndex == (int)OrderMode.Order)
                {
                    EditorGUILayout.PropertyField(props["_orderScaleY"], new GUIContent("Y Scale", "Y position will be scaled by this factor to figure out the order in layer"));
                    EditorGUILayout.PropertyField(props["_orderMin"], new GUIContent("Min. order in layer", "Minimum order in layer"));
                    EditorGUILayout.PropertyField(props["_orderMax"], new GUIContent("Max. order in layer", "Maximum order in layer"));
                }
                else
                {
                    EditorGUILayout.PropertyField(props["_orderScaleY"], new GUIContent("Y Scale", "Y position will be scaled by this factor to figure out the Z position"));
                    EditorGUILayout.PropertyField(props["_orderMinZ"], new GUIContent("Min Z", "Minimum z value"));
                    EditorGUILayout.PropertyField(props["_orderMaxZ"], new GUIContent("Max Z", "Maximum z value"));
                }
