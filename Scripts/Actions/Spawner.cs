...
public class Spawner : MonoBehaviour
{
    ...
    public void UpdateExplanation()
    {
        ...
        if (forceCount)
        {
            desc += $"This spawner will guarantee that at least {numberOfEntities} objects generated by him will \n";
            desc += "be present in the scene. When an object created by him is destroyed, another\n";
            desc += "will immediately take his place. This spawner can also be triggered manually.\n";
        }
        else
        {
            desc += $"When triggered by a [Spawn] action, this component will create new objects.\n";
            if (usePulsePattern)
            {
                desc += $"Every spawn trigger will generate {pulsePattern.Length} objects, ";
                if (pulseTime > 0) desc += $"one every {pulseTime} seconds, following a\n[{pulsePattern}] pattern, where an 'x' is a spawn, and a 'o' is a pause.\n";
                else desc += $"following a [{pulsePattern}] pattern,\nwhere an 'x' is a spawn, and a 'o' is a pause.\n";
            }
        }
        if (prefabs != null)
        {
            if (prefabs.Length > 1)
            {
                desc += "The generated object will be chosen randomly from:\n";
                foreach (var prefab in prefabs)
                {
                    if (prefab == null) desc += $"      [Nothing]\n";
                    else desc += $"      [{prefab.name}]\n";
                }
            }
            else
            {
                if (prefabs[0])
                {
                    desc += $"The generated object will be [{prefabs[0].name}].\n";
                }
                else
                {
                    desc += "There's currently no defined object to spawn.\n";
                }
            }
        }
        else
        ...
        if ((colliders != null) && (colliders.Length > 0))
        {
            if (colliders.Length > 1)
                desc += $"These objects will spawn anywhere in the area defined by any of the {colliders.Length}\nBoxCollider2D attached to this object.\n";
            else
                desc += $"These objects will spawn inside the BoxCollider2D attached to this object.\n";
        }
        else if ((spawnPoints != null) && (spawnPoints.Length > 0))
        {
            if (spawnPoints.Length > 1)
            {
                desc += "These objects will spawn on any of the positions:\n";
                foreach (var spawnPoint in spawnPoints)
                {
                    if (spawnPoint == null) desc += $"      [Nothing]\n";
                    else desc += $"      [{spawnPoint.name}]\n";
                }
            }
            else
            {
                if (spawnPoints[0])
                    desc += $"These objects will spawn will spawn at the position of [{spawnPoints[0].name}].\n";
                else
                    desc += $"These objects will spawn will spawn at the position of this object.\n";
            }
        }
        else
        {
        ...
        if (modifiers != 0)
        {
            if ((modifiers & Modifiers.Scale) != 0)
            {
                if ((modifiers & Modifiers.Speed) != 0)
                {
                    desc += $"The new objects will vary in scale from {scaleVariance.x * 100}% to {scaleVariance.y * 100}%,\n";
                    desc += $"and in speed from {speedVariance.x * 100}% to {speedVariance.y * 100}%.\n";
                }
                else
                {
                    desc += $"The new objects will vary in scale from {scaleVariance.x * 100}% to {scaleVariance.y * 100}%.\n";
            ...
            {
                desc += $"The new objects will vary in speed from {speedVariance.x * 100}% to {speedVariance.y * 100}%.\n";
            }
        }
        if (setParent)
        {
            desc += "They will also be placed as children of this object.";
        }
